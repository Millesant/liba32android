...
The result exposes guest entry/load-bias/PT_LOAD metadata plus optional `PT_DYNAMIC` guest metadata. Host reservation pointers are never part of the loader API.
...
Those belong to later linker/runtime layers and must preserve the established address-space and loader boundaries.
